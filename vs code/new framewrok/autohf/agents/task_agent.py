...
def detect_task(description: str, router: str = "auto") -> TaskInfo:
    ...
    Algorithm:
      1. Try OpenAI LLM-based task detection if requested and OPENAI_API_KEY is present
      2. Normalize input text
      3. Try exact substring match (longest keyword first)
      4. Fuzzy-match each keyword against the input
      5. Fall back to 'text-classification'
