...
def train(
    ...
    router: str = typer.Option(
        "auto",
        "--router",
        help="Routing strategy for task detection: 'auto' (use LLM if API key set), 'keyword' (local keywords), or 'openai' (force OpenAI).",
    ),
) -> None:
...
def search(
    ...
    router: str = typer.Option(
        "auto",
        "--router",
        help="Routing strategy for task detection: 'auto', 'keyword', or 'openai'.",
    ),
) -> None:
